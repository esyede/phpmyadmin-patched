...
function PMA_RTN_parseOneParameter($value)
{
    ...
    if (in_array(
        /*overload*/
        mb_strtoupper($parsed_param[$pos]['data']),
        $param_directions
    )) {
    ...
    for ($i=$pos; $i<$parsed_param['len']; $i++) {
        ...
        } elseif ($parsed_param[$i]['type'] == 'punct_bracket_open_round'
            && $depth == 0
        ) {
            $depth = 1;
        } elseif ($parsed_param[$i]['type'] == 'punct_bracket_close_round'
            && $depth == 1
        ) {
            $depth = 0;
        } elseif ($depth == 1) {
            $param_length .= $parsed_param[$i]['data'];
        } elseif ($parsed_param[$i]['type'] == 'alpha_reservedWord'
        ...
        } elseif ($parsed_param[$i]['type'] == 'alpha_columnAttrib'
            && $depth == 0
        ) {
...
function PMA_RTN_parseAllParameters($parsed_query, $routine_type)
{
    ...
    if ($parsed_query) {
        ...
        for ($i=0; $i<$parsed_query['len']; $i++) {
            ...
            } elseif ($parsed_query[$i]['type'] == 'punct_listsep' && $depth == 1) {
                $params[] = $buffer;
                $retval['num']++;
                $buffer = '';
            } elseif ($fetching == true && $depth > 0) {
                $buffer .= $parsed_query[$i]['data'] . ' ';
            }
        }
        if (! empty($buffer)) {
...
function PMA_RTN_parseRoutineDefiner($parsed_query)
{
    ...
    for ($i=0; $i<$parsed_query['len']; $i++) {
        ...
        } elseif ($fetching == true && $parsed_query[$i]['type'] == 'punct_user') {
...
function PMA_RTN_handleExecute()
{
    ...
    if (! empty($_REQUEST['execute_routine']) && ! empty($_REQUEST['item_name'])) {
        ...
        if ($routine !== false) {
            ...
            exit;
        } elseif (($GLOBALS['is_ajax_request'] == true)) {
            $message  = __('Error in processing request:') . ' ';
            $message .= sprintf(
                PMA_RTE_getWord('not_found'),
                htmlspecialchars(PMA_Util::backquote($_REQUEST['item_name'])),
