...
class PMA_TableSearch
{
     ...
     * @return string HTML elements.
     */
    private function _getForeignKeyInputBox(
        $foreignData,
        $column_name,
        $column_index,
        $titles,
     ...
     * @return string HTML elements.
     */
    private function _getEnumSetInputBox(
        $column_index,
        $criteriaValues,
        $column_type,
        $column_id,
    ...
    private function _getGeomFuncHtml($column_index)
    {
        ...
        if (in_array($this->_columnTypes[$column_index], $geom_types)) {
            ...
            $funcs = PMA_Util::getGISFunctions(
                $this->_columnTypes[$column_index],
                true,
                true
            );
            /**
    ...
    public function getSelectionForm($goto, $dataLabel = null)
    {
        ...
        if ($this->_searchType == 'zoom') {
            ...
            $html_output .= '</fieldset>';
            $html_output .= '</fieldset>';
        } elseif ($this->_searchType == 'normal') {
            ...
            $html_output .= '</fieldset>';
        } elseif ($this->_searchType == 'replace') {
     ...
     */
    public function _getSearchAndReplaceHTML()
    {
        $htmlOutput  = __('Find:')
     ...
     */
    public function _getRegexReplaceRows($columnIndex, $find, $replaceWith, $charSet)
    {
