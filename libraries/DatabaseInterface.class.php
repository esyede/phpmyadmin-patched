...
class PMA_DatabaseInterface
{
    ...
    public static function checkDbExtension($extension = 'mysql')
    {
        if ($extension == 'drizzle' && function_exists('drizzle_create')) {
            return true;
        } elseif (function_exists($extension . '_connect')) {
            return true;
        }
        return false;
    }
    ...
    public function getTablesFull(
        ...
        if (! $GLOBALS['cfg']['Server']['DisableIS']) {
            ...
            } elseif ($sort_by == 'Data_length') { // Size = Data_length + Index_length
                foreach ($tables as $one_database_name => $one_database_tables) {
                    uasort(
                        $one_database_tables,
                        function ($a, $b) {
    ...
    public function isUserType($type)
    {
        ...
        if (! $GLOBALS['cfg']['Server']['DisableIS'] || $type === 'super') {
            ...
            if ($type === 'super') {
                ...
                $userAndHost = $this->_getCurrentUserAndHost();
                $user = isset($userAndHost[0]) ? $userAndHost[0] : '';
                $host = isset($userAndHost[1]) ? $userAndHost[1] : '';
