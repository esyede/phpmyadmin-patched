...
include_once __DIR__ .  '/strftime.php';
...
class PMA_Util
{
    ...
    public static function localisedDate($timestamp = -1, $format = '')
    {
        ...
        $date = preg_replace(
            '@%[aA]@',
            $day_of_week[(int)polyfill_strftime('%w', $timestamp)],
            $format
        );
        $date = preg_replace(
            '@%[bB]@',
            $month[(int)polyfill_strftime('%m', $timestamp)-1],
            $date
        );

        $ret = polyfill_strftime($date, $timestamp);
    ...
    /**
     * Formats user string, expanding @VARIABLES@, accepting polyfill_strftime format
     * string.
     *
    ...
    public static function expandUserString(
        ...
        /* Do the replacement */
        return strtr(polyfill_strftime($string), $replace);
    }
