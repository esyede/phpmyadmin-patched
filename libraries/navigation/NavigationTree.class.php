...
class PMA_NavigationTree
{
    ...
    public function __construct()
    {
        ...
        } elseif (isset($_REQUEST['n0_aPath'])) {
            $count = 0;
        ...
        } elseif (isset($_REQUEST['n0_vPath'])) {
            $count = 0;
    ...
    private function _getNavigationDbPos()
    {
        ...
        if (! $GLOBALS['cfg']['Server']['DisableIS']) {
            $dbSeparator = PMA_Util::sqlAddSlashes(
    ...
    public function groupNode($node)
    {
        ...
        foreach ($prefixes as $key => $value) {
            if ($value == 1) {
                unset($prefixes[$key]);
            } elseif ($value > 500 && ! $this->_largeGroupWarning) {
                trigger_error(
                    __(
    ...
    private function _fastFilterHtml($node)
    {
        ...
        if ($node === $this->_tree
            ...
            $retval .= "</form>";
            $retval .= "</li>";
        } elseif (($node->type == Node::CONTAINER
    ...
    private function _getPageSelector($node)
    {
        ...
        } elseif ($node->type == Node::CONTAINER && ! $node->is_group) {
            $paths = $node->getPaths();
